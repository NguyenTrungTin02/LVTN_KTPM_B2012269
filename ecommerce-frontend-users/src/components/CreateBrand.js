...
import { apiAddBrand, apiAddCategory } from '../apis';
import {Loading} from '../components';

const CreateBrand = ({setIsShowAddBrand}) => {
    ...
    const handleAdd= async (data)=>{
        ...
        const response = await apiAddBrand(fromData)
        setLoading(false)
        if(response.success) toast.success(response.mes)
        else toast.error(response.mes)
        setIsShowAddBrand(false)
    }
  ...
  return (
    <div className='flex justify-center items-center mt-[80px]'>
        ...
        <div className='flex justify-end cursor-pointer'>
          <IoMdClose size={24} onClick={() => setIsShowAddBrand(false)} />
        </div>

        <div className='border-b w-full items-center justify-center pt-4 pb-4 flex'>
          <h5 className='text-xl font-bold tracking-tight'>Thêm thương hiệu</h5>
        </div>
        ...
        <div className='p-4'>
          ...
          {preview.thumb && <div className='my-4'>
            <img src={preview.thumb} alt="thumb" className="w-[200px] object-contain"/>
                ...
                <Button style='px-4 py-2 rounded-md text-white text-semibold bg-gray-500 my-2' handleOnclick={()=>setIsShowAddBrand(false)}>Hủy</Button>
